...
def findAllLogosForCode(logoRoot, decorationCode):
    """
    Find all logo files matching a decoration code.
    Useful for debugging or when multiple formats exist.
    
    Returns:
        List of matching file paths
    """
    if not decorationCode or not logoRoot or not os.path.exists(logoRoot):
        return []
    ...
    for root, _, files in os.walk(logoRoot):
        for f in files:
            ...
            nameNorm = normalize(name)
            if codeNorm == nameNorm or codeNorm in nameNorm:
