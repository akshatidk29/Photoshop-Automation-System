...
# Logo processing cache: (absPath, angle, targetSize) -> temp file path
_processedLogoCache = {}
...
def _copyLogoToClipboard(app, actualLogoPath):
    """Open logo, copy to clipboard, close. Simple and safe for COM."""
    logoDoc = app.Open(actualLogoPath)
    logoDoc.Selection.SelectAll()
    logoDoc.Selection.Copy()
    logoDoc.Close(2)  # 2 = psDoNotSaveChanges


def rotateLogo(logoPath, angle, target_size=None):
    """Rotate logo image by specified angle, handling PDFs. Results are cached."""
    from pathlib import Path
    
    # Check cache first
    cacheKey = (os.path.abspath(logoPath), angle, target_size)
    if cacheKey in _processedLogoCache:
        cachedPath = _processedLogoCache[cacheKey]
        if os.path.exists(cachedPath):
            return cachedPath
        # Cached file was deleted, remove stale entry
        del _processedLogoCache[cacheKey]
    ...
    # Read logo image with alpha channel, preserving color profile using Pillow
    try:
        pil_img = Image.open(logoPath)
        # Convert to RGBA (handles CMYK -> RGB conversion internally)
        pil_img = pil_img.convert("RGBA")
        # Convert to numpy array for OpenCV compatibility (BGR standard for cv2)
        # Pillow uses RGB, OpenCV uses BGR
        img_array = np.array(pil_img)
        # Convert RGB to BGR for OpenCV
        image = cv2.cvtColor(img_array, cv2.COLOR_RGBA2BGRA)
    except Exception as e:
        print(f"Error loading logo with Pillow: {e}, falling back to OpenCV")
    ...
    # Cache the result
    _processedLogoCache[cacheKey] = temp_file.name
...
def preparePairDoc(imagePath, logoPath, locationName, coordinates, rotation,
    ...
    """Create Photoshop document with image and placed logo."""
    try:
        ...
        # Copy logo to clipboard (reuse if same)
        _copyLogoToClipboard(app, actualLogoPath)
...
def prepareComboPairDoc(imagePath, logoPathsList, positionsList, coordinatesList, rotationsList,
                         garmentType, customLogoSizes, decorationCode, targetName, canvasHeight=1200,
                         clippingEnabled=False, clippingPositions=None):
    """Create Photoshop document with image and multiple logos at different positions."""
    try:
        app = _getPhotoshopApp()
        ...
        # Ensure we have enough logo paths for all positions (fallback to first valid)
        validLogoPaths = [lp for lp in logoPathsList if lp]
        if validLogoPaths:
            while len(logoPathsList) < len(positionsList):
                cycleIdx = len(logoPathsList) % len(validLogoPaths)
                logoPathsList.append(validLogoPaths[cycleIdx])

        # Place logo at EACH position with corresponding logo from list
        for idx, (position, coordinates) in enumerate(zip(positionsList, coordinatesList)):
            ...
            # Get the logo path for THIS position
            currentLogoPath = logoPathsList[idx] if idx < len(logoPathsList) else (validLogoPaths[0] if validLogoPaths else None)
            if not currentLogoPath:
                print(f"    [WARN] Skipping position {position} - no logo path")
                continue
            ...
            logo_was_clipped = False
            actualLogoPath = currentLogoPath  # Use the specific logo for this position

            if should_clip:
                try:
                    rot_angle = rotation if rotation else 0
                    actualLogoPath = rotateLogo(currentLogoPath, rot_angle, target_size=targetLogoWidth)
